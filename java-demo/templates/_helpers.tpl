...
{{- if and .Values.cloudSQLProxy.enabled (ne .Values.env "sandbox") }}
...
{{- else if and .Values.cloudSQLProxy.enabled (eq .Values.env "sandbox") }}
{{- printf "serai-traceability-%s:asia-east2:serai-traceability-asea2-pgsql-%s-%s" .Values.env .Values.env .Values.cloudSQLProxy.instance_no }}
{{- end }}
{{- end }}

{{/*
Create the name of the db instance ID to use
*/}}
{{- define "app-service.dbInstanceId" -}}
{{- if and .Values.cloudSQLProxy.enabled (ne .Values.env "sandbox") }}
{{- printf "serai-traceability-asea2-pgsql-%s-%s" .Values.env .Values.cloudSQLProxy.instance_no .Values.env }}
{{- else if and .Values.cloudSQLProxy.enabled (eq .Values.env "sandbox") }}
{{- printf "serai-traceability-asea2-pgsql-%s-%s" .Values.env .Values.env .Values.cloudSQLProxy.instance_no }}
{{- end }}
{{- end }}
